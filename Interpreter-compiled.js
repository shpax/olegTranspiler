...
var Interpreter = function () {
    ...
    _createClass(Interpreter, [{
        ...
        value: function parseLine(line) {
            var newLine = line.replace(/Oleg\s([^\s]+)\szovut/g, 'var $1 =').replace(/oleshka/g, 'function').replace(/menya\./g, 'document.').replace(/\.zovut/g, '.querySelector').replace(/\.zvat/g, '.querySelectorAll').replace(/\szovut\s/g, ' = ').replace(/.menya/g, '.innerHTML').replace(/OLEG/g, 'alert').replace(/yoyoyo/g, '[].slice.call').replace(/\.vseh/g, '.forEach').replace(/YoYo/g, '{').replace(/yoyo/g, '}').replace(/Yo/g, '(').replace(/yo/g, ')');
